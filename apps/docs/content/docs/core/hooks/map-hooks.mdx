...
Returns a function that returns a Promise which resolves when the map is ready. This is useful for async operations that need to wait for the map to be fully loaded before proceeding.
...
function AsyncAction() {
  ...
  const handleFly = async () => {
    ...
    // Now safe to use map methods
    controller.map.flyTo({ center: [0, 0], zoom: 5 });
  };

  return <button onClick={handleFly}>Fly Home</button>;
}
```
